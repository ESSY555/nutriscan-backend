...
class MealController extends Controller
{
    ...
    public function context(Request $request): JsonResponse
    {
        ...
        $timeContext = $data['timeContext'] ?? 'breakfast';
        $country = $data['country'] ?? null;
        $goal = $data['goal'] ?? null;

        $meals = $this->fetchMealsForContext($diet, $allergens, $timeContext, $country, $goal);
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        $data = $this->fetchJson('/search.php', ['s' => $q]);
        $meals = ($data['meals'] ?? []);
        return response()->json(['meals' => is_array($meals) ? $meals : []]);
    }
    ...
    public function show(string $id): JsonResponse
    {
        ...
        if (!$meal) {
            return response()->json(['meal' => null], 404);
        }
        return response()->json(['meal' => $meal]);
    }

    private function fetchMealsForContext(string $diet, array $allergens, string $timeContext, ?string $country, ?string $goal): array
    {
        $baseList = [];
        ...
        return count($safe) ? $safe : array_slice($shuffled, 0, 12);
    }
    ...
    {
        try {
            $response = Http::timeout($this->timeoutSeconds)->get($this->base . $path, $query);
            if ($response->ok()) {
